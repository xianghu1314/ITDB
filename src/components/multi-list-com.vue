...
<div id="multi-list-com">
    <div v-for="item in formatList" >
        ...
        <div v-else-if="item.type==4" class="weui-flex list-type4" >
            <div class="weui-flex__item" @click="gotoDetail(it)">
                <img  :src="getArray(item.data,0).logo">
            </div>
            <div class="weui-flex__item" @click="gotoDetail(it)">
                <div class="weui-flex__item_footer">
                    <h5>{{getArray(item.data,0).title}}</h5>
                    <p>{{getArray(item.data,0).describes}}</p>
                    <div class="item_footer_bottom">
                        <span>¥{{getArray(item.data,0).Prise}}</span>
                        <span>夺宝{{getArray(item.data,0).LotteryPrise}}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-else-if="item.type==7" class="weui-flex list-type7">
            ...
            <div class="weui-flex__item list-two" >
                <div @click="gotoDetail(getArray(item.data,1))">
                    <img  :src="getArray(item.data,1).logo">
                </div>
                <div @click="gotoDetail(getArray(item.data,2))">
                    <img  :src="getArray(item.data,2).logo">
                </div>
            </div>
        </div>
        ...
        <div v-else-if="item.type==9" class="weui-flex list-type9">
            ...
            <div class="item-sm" @click="gotoDetail(getArray(item.data,1))">
                ...
                <div class="weui-flex__item_footer">
                    ...
                    <p>{{getArray(item.data,1).describes}}</p>
                    <div class="item_footer_bottom">
                        <span>¥{{getArray(item.data,1).Prise}}</span>
                        <span>夺宝{{getArray(item.data,1).LotteryPrise}}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            gotoDetail:function (item) {
                this.$router.push({path:'/goodsDetail',query: { id: 1 }});
            }
        ...
        computed: {
            formatList:function () {
                ...
                console.log("传过来的数据"+self.ArrayData.length)
                console.log("老数据"+self.ItemList.length)
                //新增的数据
                arr.splice(0,self.ItemList.length);
                //保存数据
                self.ItemList=[].concat(self.ArrayData);

                console.log("新增的数据"+arr.length)
                //获取默认节点
                self.CurrentClassType=self.ClassType[6];//6
                var arr2=arr.splice(0,self.CurrentClassType.num);
                while (arr.length>0){
                    var it={data:arr2,type:self.CurrentClassType.type}
                    self.FormatList.push(it);
                    var random=self.getRandom();
                    console.log("随机数"+random);

                    self.CurrentClassType=self.ClassType[random];
                    arr2=arr.splice(0,self.CurrentClassType.num);
                }
